...
export class ProductsState {
    ...
    @Action(AddProductPhotos)
    addProductPhotos(ctx: StateContext<ProductsStateModel>, {id, payload}: AddProductPhotos) {
        console.log("Adding photos")

        return this._productService.addPhotos(id, payload).pipe(tap(returnData => {

            const state = ctx.getState();
            const products = [...state.products];
            const index = products.findIndex(x => x.id == id);
            products[index].photos = returnData;
